...
from imageReader import readText
...
def makePuzzle(length,width,filled):
	...
	return (puzzle, across, down)

def determineData(size, across, down, puzzle):
	acrossDict = {}
	for index in across:
		for i in range(0,size*size):
			if puzzle[i]==index:
				newIndex = i
				containedSpots = [newIndex]
				while(newIndex<size*size-1 and newIndex%size!=size-1 and puzzle[newIndex+1]!="*"):
					newIndex+=1
					containedSpots.append(newIndex)
		acrossDict[index] = containedSpots
	downDict = {}
	for index in down:
		for i in range(0,size*size):
			if puzzle[i]==index:
				newIndex = i
				containedSpots = [newIndex]
				while(newIndex<size*size-size and puzzle[newIndex+size]!="*"):
					newIndex+=size
					containedSpots.append(newIndex)
		downDict[index] = containedSpots
	return(acrossDict,downDict)

def reverseDict(originalDict):
	newDict = {}
	for key in originalDict:
		for index in originalDict[key]:
				newDict[index] = key
	print(newDict)
...
def getGrid(picture):
	...
	(puzzle, across, down) = makePuzzle(len(horizons)-1,len(verticals)-1,filled)

	print(puzzle)

	print()
	print()
	(acrossDict,downDict) = determineData(len(horizons)-1, across, down, puzzle)
	reverseAcrossDict = reverseDict(acrossDict)
	reverseDownDict = reverseDict(downDict)
	...
	(acrossClueDict,downClueDict) = readText('newBlank.jpg')

	for key in acrossDict:
		if key not in acrossClueDict:
			acrossClueDict[key]=''
	for key in downDict:
		if key not in downClueDict:
			downClueDict[key]=''
	delete = []
	for key in acrossClueDict:
		if key not in acrossDict:
			delete.append(key)
	for key in delete:
		del acrossClueDict[key]
	delete = []
	for key in downClueDict:
		if key not in downDict:
			delete.append(key)
	for key in delete:
		del downClueDict[key]

	'''print("ACROSS\n")
	for clue in acrossClueDict:
		print(clue, ": ", acrossClueDict[clue])
	print("\n\nDOWN\n")
	for clue in downClueDict:
		print(clue, ": ", downClueDict[clue])'''

	print("\n\n")
	print(acrossClueDict)
	print("\n\n")
	print(downClueDict)
