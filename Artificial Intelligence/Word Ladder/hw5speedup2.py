...
alreadyCounted = set([])
for key in adjacencyList:
   nums = len(adjacencyList[key])
   if(nums==0 or nums==1 or nums == 2):
      alreadyCounted.add(key)
for key in adjacencyList:
   #print count
   if key not in alreadyCounted:
      farWord = furthestWord(key, adjacencyList)
      pathTo = BFS(key, farWord, adjacencyList)
      diameterHelper = len(pathTo)
      for word in pathTo:
         if word not in alreadyCounted:
            count += 1
            alreadyCounted.add(word)
      if(diameterHelper>diameter):
         diameter = diameterHelper'''
...
for key in adjacencyList:
   ...
   if(nums!=1):
      alreadyCounted.add(key)
for word in adjacencyList:
   if word not in alreadyCounted:
      farWord = furthestWord(word, adjacencyList)
      pathTo = BFS(word, farWord, adjacencyList)
      diameterHelper = len(pathTo)-1
      for word in pathTo:
         if word not in alreadyCounted:
            alreadyCounted.add(word)
      if(diameterHelper>diameter):
...
for key in adjacencyList:
   if(len(adjacencyList[key]) == maxLength):
      farWord = furthestWord(key, adjacencyList)
      farWordTwo = furthestWord(farWord, adjacencyList)
      diameterHelper = len(BFS(farWord, farWordTwo, adjacencyList))-1
      if(diameterHelper>diameter):
         diameter = diameterHelper
...
#   else:
#      print "The path between ", startWord, " and ", endWord, " is of length ", len(pathTo), \
#         " and it is ", pathTo
