...
def compChoose3Helper(board, turnLetter, level):
	opp = 1
	if level%2 == 0: opp = -1
	if level > 2: return compVal(board, turnLetter)
	level += 1
	possibleMoves = findPossibilities(board, turnLetter)
	maxVal = maxInd = -1*opp*1000000
	for tup in possibleMoves:
		index, num = tup
		value = opp*compChoose3Helper(inputSpot(board, index, turnLetter), turnLetter, level)
		if value > maxVal and opp == 1:
			maxInd = index
			maxVal = value
		if value < maxVal and opp == -1:
			maxInd = index
			maxVal = value
	if maxInd <0 and opp == 1:
		return compVal(board, turnLetter)
	if maxInd >100000 and opp = -1:
		return compVal(board, turnLetter)
...
print(compChoose3(board, turnLetter))
